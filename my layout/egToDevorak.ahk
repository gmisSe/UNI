...
for name in ["Code.exe", "devenv.exe", "Obsidian.exe"] {
...
{
    global normalMode := 0
    ih := InputHook("V",, "w,c,iw,e,$,Backspace")
    ih.Start()
    ih.Wait(2)
    ih.Stop()
    global normalMode := 1
    try if ih.EndReason = "Match"
        insertModeF()
}
...
{
    global normalMode := 0
    ih := InputHook("V",, "w,d,iw,aw,$,Backspace")
    ih.Start()
    ih.Wait(2)
    ih.Stop()
    global normalMode := 1
...
{
    global normalMode := 0
    ih := InputHook("V",, "w,y,iw,aw,$,Backspace")
    ih.Start()
    ih.Wait(2)
    ih.Stop()
    global normalMode := 1
...
{
    global normalMode := 0
    ih := InputHook("V C",, "wip,_,u,J,U,~,g,d,D,p,P,t,T,,,,;,Backspace")
    ih.Start()
    ih.Wait(2)
    ih.Stop()
    global normalMode := 1
...
#HotIf WinActive("ahk_exe Code.exe") or WinActive("ahk_exe devenv.exe") or WinActive("ahk_exe Obsidian.exe")
~Esc::
{
    normalModeF()
}
